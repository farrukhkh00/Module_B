...
document.addEventListener('DOMContentLoaded', showMobileBrands);

let brandSelect = document.getElementById('brand');
  
  
function showMobileBrands() {
    let brand_options = '<option value="">--Select Brand--</option>'
  
    Object.keys(mobiles).forEach(brand => {
      let optionsText = brand[0].toUpperCase() + brand.slice(1)
  
      brand_options += `<option value="${brand}">${optionsText}</option>`;
    });
  
    brandSelect.innerHTML = brand_options;
  }
  

function showMobileModels() {
    let modelSelect = document.getElementById('model');
    let selectedBrand = brandSelect.value;
  
    let model_options = '<option value="">--Select Model--</option>';
  
    let models = mobiles[selectedBrand];
      Object.keys(models).forEach(model =>{
        let modelOptionsText = model[0].toUpperCase() + model.slice(1)
        
        model_options += `<option value="${model}">${modelOptionsText}</option>`;
      }
      )
...
function showDetails() {
  ...
  let selectedModel = modelSelect.value;
      ...
      let mobilesBrandKeys = Object.keys(mobiles[selectedBrand][selectedModel])
      let mobilesBrandValues = Object.values(mobiles[selectedBrand][selectedModel])
      
      for(let i = 0; i < mobilesBrandKeys.length; i++ ){
      detailsDiv.innerHTML += `<p>${mobilesBrandKeys[i]}: ${mobilesBrandValues[i]} </p>`

      
      if (typeof  mobilesBrandValues[i - 1] == "object"){
        let array = document.getElementsByTagName('p')

      
        let array2 = array[i - 1].innerHTML.slice(array[i - 1].innerHTML.indexOf(":"))
        let array3 = array2.replaceAll("[object Object]",JSON.stringify(mobilesBrandValues[i - 1]))
        let array4 = array[i - 1].innerHTML.slice(0,array[i - 1].innerHTML.indexOf(":"))
        detailsDiv.innerHTML += array[i - 1].innerHTML.replace("[object Object]",array3)   
        
        detailsDiv.innerHTML = detailsDiv.innerHTML.replaceAll(`${array4}: [object Object]`,"")
